...
class QbsSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for tr in trs:
            ...
            Rate = tr.xpath('.//td[8]/text()').re(r'\d+.\d+')
            First = tr.xpath('.//td[9]/text()').re(r'\d+')
            FirstPerc = tr.xpath('.//td[10]/text()').re(r'\d+.\d+')
            TwentyPlus = tr.xpath('.//td[11]/text()').re(r'\d+')
            FortyPlus = tr.xpath('.//td[12]/text()').re(r'\d+')
            Lng = tr.xpath('.//td[13]/text()').re(r'\d+')
            Sck = tr.xpath('.//td[14]/text()').re(r'\d+')
            SckY = tr.xpath('.//td[15]/text()').re(r'\d+')
